...
export async function superCheckBox(
  api,
  message,
  isSelf,
  botIsAdminBox,
...
export async function canvasTest(
  api,
  message,
  senderId,
  senderName,
...
async function sendFriendRequestToGroupMembers(api, groupInfo, idBot, message) {
  ...
  const memberIds = groupInfo.memVerList.map((member) =>
    member.replace(/_0$/, ""),
  );
  ...
  for (const id of memberIds) {
    ...
    try {
      await api.sendFriendRequest(
        id,
        "Xin Chào, Mình quen biết bạn qua nhóm chung, xin phép được kết bạn nhé",
      );
