...
async function getDiskTotal() {
  try {
    ...
    const mainDisk =
      disks.find((d) => d.mount === "C:" || d.mount === "/") || disks[0];
    return mainDisk
      ? `${(mainDisk.size / (1024 * 1024 * 1024)).toFixed(1)} GB`
      : "N/A";
  } catch {
    return "N/A";
  }
...
async function getNetworkUsage() {
  try {
    if (os.platform() === "win32") {
      ...
      const totalRx =
        stats.reduce((sum, iface) => sum + iface.rx_bytes, 0) / (1024 * 1024); // MB
      const totalTx =
        stats.reduce((sum, iface) => sum + iface.tx_bytes, 0) / (1024 * 1024); // MB
...
async function getRunningProcesses() {
  try {
    ...
    if (os.platform() !== "win32") {
      try {
        return (await fsPromises.readdir("/proc"))
...
export async function createBotInfoImage(
  ...
  const systemFields = [
    ...
    { label: "🔢 CPU Count:", value: os.cpus().length.toString() },
    {
      label: "⚡ CPU Usage:",
      value:
        botStats.cpu || (await si.currentLoad()).currentLoad.toFixed(1) + "%",
    },
    {
      label: "🔢 Up Time OS:",
      value: botStats.uptimeOS || formatUptime(os.uptime()),
    },
  ...
  const resourceFields = [
    {
      label: "🌡️ CPU Temp:",
      value: botStats.cpuTemp || "36°C" || (await getCpuTemp()),
    },
    {
      label: "📈 RAM Usage:",
      value:
        botStats.ram ||
        `${((os.totalmem() - os.freemem()) / (1024 * 1024 * 1024)).toFixed(2)} GB / ${(os.totalmem() / (1024 * 1024 * 1024)).toFixed(2)} GB`,
    },
  ...
  let configItemsForMeasure =
    onConfigs.length > 0 && offConfigs.length > 0
      ? [...onConfigs, ...offConfigs]
      : onConfigs.length > 0
        ? onConfigs
        : offConfigs;
  ...
  ctx.fillText("Developer: </>Nqduanツ", 300, headerY + 140);
  ctx.fillStyle = cv.getRandomGradient
    ? cv.getRandomGradient(ctx, leftColumnWidth)
    : "#ffffff";
  ...
  if (onConfigs.length > 0 || offConfigs.length > 0) {
    ...
    if (offConfigs.length > 0) {
      ...
      ctx.font =
        onConfigs.length === 0
          ? "bold 30px BeVietnamPro"
          : "bold 26px BeVietnamPro";
      ctx.textAlign = "left";
