...
import { getDataAllGroup } from "../Nqduan-service/info-service/group-info.js";
...
export async function startWebServer(api) {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("getAllFriends", async () => {
      try {
        ...
        if (
          !cachedFriends ||
          currentTime - lastFriendsFetchTime > CACHE_DURATION
        ) {
    ...
    socket.on("getAllGroups", async () => {
      try {
        ...
        if (
          !cachedGroups ||
          currentTime - lastGroupsFetchTime > GROUPS_CACHE_DURATION
        ) {
    ...
    socket.on(
      "updateActiveBotStatus",
      async ({ groupId, groupName, isActive }) => {
        try {
          await changeStatusConfig({
            api,
            groupId,
            groupName,
            command: "activeBot",
    ...
    socket.on(
      "updateFutureStatus",
      async ({ groupId, groupName, command, isActive }) => {
        try {
          await changeStatusConfig({
            api,
            groupId,
    ...
    socket.on("disconnect", () => {
      console.log(
        chalk.red(`Client ${socket.id} đã ngắt kết nối khỏi web socket`),
      );
  ...
  httpServer.listen(PORT, () => {
    console.log(
      chalk.yellow(`Web server đã khởi tạo thành công trên port ${PORT}`),
    );
  });
}
