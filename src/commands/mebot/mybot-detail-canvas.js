...
import { getUserInfoData } from "../../../src/Nqduan-service/info-service/user-info.js";
...
export async function createMyBotDetailImage(
  botInfo,
  pm2Status,
  createdAt,
  expiryAt,
  ...
  const botFields = [
    // Giữ nguyên logic hiển thị text
  ...
  try {
    // ================== THAY ĐỔI DUY NHẤT ==================
    // Đổi từ botInfo.name sang botInfo.uid_bot
    const userInfo = await getUserInfoData(api, botInfo.uid_bot);
    // ================== KẾT THÚC THAY ĐỔI ==================
    ...
    // Sửa log lỗi để dùng uid_bot
    console.error(
      `Lỗi lấy avatar cho bot ${botInfo.uid_bot}: ${error.message}`,
    );
  }
