...
async function initSession() {
  ...
  try {
    ...
    console.log(
      `Cookies after POST /logininfo: ${postResponse.headers.get('set-cookie') || 'none'}`
    );
...
async function verifyClient(session) {
  ...
  try {
    ...
    console.log(
      `Cookies after POST /verify-client: ${response.headers.get('set-cookie') || 'none'}`
    );
    return session;
  } catch (e) {
      ...
      if (statusData.error_code === 0) {
        ...
        const rawCookies = {
          ...
          cookies: allCookies.map(cookie => ({
            ...
            expirationDate:
              cookie.expires instanceof Date && !isNaN(cookie.expires.getTime())
