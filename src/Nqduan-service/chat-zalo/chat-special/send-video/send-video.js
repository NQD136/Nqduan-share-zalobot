...
const VIDEO_TYPES = {
  girl: {
    variants: {
      default: { source: "vdgirl.txt", ttl: 3600000 },
      sexy: { source: "vdsexy.txt", ttl: 3600000, type: "Sexy" },
    },
  },
  sexy: {
    variants: {
      default: { source: "vdsexy.txt", ttl: 3600000 },
    },
  },
  anime: {
    variants: {
      default: { source: "vdanime.txt", ttl: 3600000 },
    },
  },
  tet: {
    variants: {
      default: { source: "vdtet.txt", ttl: 336000000 },
    },
  },
  cosplay: {
    variants: {
      default: { source: "vdcos.txt", ttl: 3600000 },
    },
  },
  gai: {
    variants: {
      default: { source: "vdgai.txt", ttl: 3600000 },
    },
  },
  boy: {
    variants: {
      default: { source: "vdboy.txt", ttl: 3600000 },
    },
  },
  chill: {
    variants: {
      default: { source: "vdchill.txt", ttl: 3600000 },
    },
  },
  vuto: {
    variants: {
      default: { source: "vdvuto.txt", ttl: 3600000 },
    },
  },
  sad: {
    variants: {
      default: { source: "vdsad.txt", ttl: 3600000 },
    },
  },
  sex: {
    variants: {
      default: { source: "vdsex.txt", ttl: 3600000 },
    },
  },
};
...
async function handleApiSourceVideo(
  api,
  message,
  config,
  senderName,
...
async function handleApiExternalVideo(
  api,
  message,
  config,
  senderName,
  ...
  while (retryCount < CONFIG.maxRetries) {
    try {
      ...
      if (response.status !== 200)
        throw new Error("Không thể lấy dữ liệu từ source");
...
export const handleVideoCommand = async (api, message, type) => {
  ...
  if (config.variantConfig.api) {
    success = await handleApiExternalVideo(
      api,
      message,
      config,
      senderName,
    ...
    success = await handleApiSourceVideo(
      api,
      message,
      config,
      senderName,
...
export async function sendRandomGirlVideo(
  api,
  message,
  caption,
  type,
