...
export async function drawShopCanvas(landPrice) {
  ...
  const height =
    (ITEM_HEIGHT + PADDING) * (toolRows + seedRows) +
    PADDING * 3 +
    HEADER_HEIGHT * 2 +
    SECTION_PADDING;
...
async function drawItem(ctx, [key, item], x, y, landPrice) {
  ...
  if (productCode) {
    ...
    if (productImage) {
      const scale = Math.min(
        IMAGE_SIZE / productImage.width,
        IMAGE_SIZE / productImage.height,
      );
  ...
  if (item.seedPrice) {
    ...
    ctx.fillText(
      `💰 ${item.seedPrice.toLocaleString()}đ`,
      x + ITEM_WIDTH / 2,
      imageY + 70,
    );
    ...
    ctx.fillText(
      `💎 ${item.harvestValue.toLocaleString()}đ`,
      x + ITEM_WIDTH / 2,
      imageY + 140,
    );
  } else {
    ...
    ctx.fillText(
      `💰 ${formatCurrency(price, 999_999_999_999)} VND`,
      x + ITEM_WIDTH / 2,
      imageY + 70,
    );
