...
export async function handleChatBot(api, message, threadId, groupSettings, nameGroup, isHandleCommand) {
  ...
  // ✅ THAY ĐỔI 1: Lấy prefix động
  const prefix = getGlobalPrefix();

  if (
    groupSettings[threadId].replyEnabled &&
    // ✅ THAY ĐỔI 1: Chỉ check prefix động, bỏ check '!' và '.'
...
export async function handleLearnCommand(api, message, groupSettings) {
  ...
  if (content.startsWith(`${prefix}learnnow_`)) {
    ...
    if (parts.length >= 3) {
      ...
      const caption = `Cú pháp Không hợp lệ. Vui lòng sử dụng: ${prefix}learnnow_[Câu Hỏi]_[Câu Trả Lời] để dạy bot học câu trả lời mới`;
      await sendMessageWarning(api, message, caption);
    }
    return true;
    ...
    if (parts.length === 1) {
      ...
      await sendMessageWarning(api, message, `❌ Cú pháp Không hợp lệ. Sử dụng ${prefix}learn, ${prefix}learn on/off để bật tắt chế độ học tập`);
    }
    return true;
...
export async function handleReplyCommand(api, message, groupSettings) {
  ...
  if (content.startsWith(`${prefix}reply`)) {
    ...
    if (parts.length === 1) {
      ...
      await sendMessageWarning(api, message, `Cú pháp Không hợp lệ. Sử dụng ${prefix}reply hoặc ${prefix}reply on/off để bật tắt chế độ trả lời`);
    }
    return true;
  }
  return false;
...
export async function handleUnlearnCommand(api, message) {
  ...
  if (content.startsWith(`${prefix}unlearn`)) {
    ...
    if (parts.length >= 2) {
      ...
      await api.sendMessage(
        {
          msg: `❌ Cú pháp Không hợp lệ. Vui lòng sử dụng: ${prefix}unlearn [Câu Trả Lời] để xóa câu hỏi tương ứng`,
          quote: message,
          ttl: 30000,
        },
        threadId,
