// src/Nqduan-service/api-crawl/tiktok/tiktok-api.js
...
const headersSearch = {
  ...
  cookie:
    "_ttp=2p0lHkLPkuuzPy4n9kNCgaqVBFM; tt_chain_token=tioPB+Lui7TvHuzUPNaOHg==; passport_csrf_token=3dfb5a52d5a68f4d5c9bba4538577dfe; passport_csrf_token_default=3dfb5a52d5a68f4d5c9bba4538577dfe; odin_tt=0a5b7e2bc3c60961f0e9463f055e859c8ca0059ae611097cd3dd3f4f259a70c0ec1343ec5dfb12f6361658cbac0ad226f69c1b6cba2089e7f174c7f9388708e449330d0c262c262c38701c9381fc9a679cae; ttwid=1%7CaIhJLRa4fvQV5lLYGHleEtravH48pseLFrAf8dwU0ik%7C1736404109%7Ce5b777f06b45e5add32991c13b233b1867fa01f50ba08d37f095ee96b81648c4; msToken=ruVh1eb2RQfH8sIdNQMUgirrpsBRM7lyjGt_nJbm8DVQXSxVKiz4L73PlHcZ_AVXhZu5qRBI5DoAuagqgXmZohLuz7vtDEF-g4EQwW8aB9tcv1q1nZhPwnEcwtFHCTN2SIlrRdJREgK_9OqqxttLqrPLpw==",
  "Accept-Language": "vi-VN,vi;q=0.9",
};
const clientABVersions =
  "70508271,72923696,73022009,73038832,73060926,73067877,73119041,73149840,73158100,73167672,73174521,73184446,73184714,73197619,73198009,73204427,73216053,73234258,73242625,73242627,73242629,70405643,71057832,71200802,72258247,73004916,73171280,73208420";

const tiktokApi = "https://m.tiktok.com/api";
const tiktokvApi = `https://api16-normal-useast5.tiktokv.us`;
const tiktokvFeed = (params) =>
  `${tiktokvApi}/aweme/v1/feed/?${tiktokApiParams(params)}`;
const linkTiktokSearch = (keywords) =>
  `${tiktokApi}/search/general/full/?keyword=${encodeURI(keywords)}`;
const linkTiktokRelated = (idVideo) =>
  `${tiktokApi}/related/item_list/?${tiktokRelatedParams(idVideo)}`;
const linkTiktokPreload = () =>
  `${tiktokApi}/preload/item_list/?${tiktokPreloadParams()}`;
const linkTiktokRecommend = () =>
  `${tiktokApi}/recommend/item_list/?${tiktokRecommendParams()}`;
const linkTikwm = "https://www.tikwm.com/api/?url=";
const linkSearchTiktokTikwm = (keywords) =>
  `${linkTikwm}keywords=${encodeURI(keywords)}`;
...
const parseVideoData = (content) => {
  const data = {
    ...
    video: {
      url: content.video.play_addr.url_list[
        content.video.play_addr.url_list.length - 1
      ],
      cover:
        content.video.cover.url_list[content.video.cover.url_list.length - 1],
    ...
    music: {
      ...
      url: content.music.play_url.url_list[
        content.music.play_url.url_list.length - 1
      ],
      cover:
        content.music?.cover_hd?.url_list[
          content.music.cover_hd.url_list.length - 1
        ] ||
        content.music?.cover_large?.url_list[
          content.music.cover_large.url_list.length - 1
        ] ||
        content.music?.cover_medium?.url_list[
          content.music.cover_medium.url_list.length - 1
        ] ||
        content.music?.cover_thumb?.url_list[
          content.music.cover_thumb.url_list.length - 1
        ],
...
export async function searchTiktokTikwm(keywords) {
  try {
    const response = await axios.get(linkSearchTiktokTikwm(keywords), {
      timeout: 10000,
    });
    ...
    console.error(
      "Error while fetching search data from tikwm:",
      error.message,
    );
    return [];
  }
...
export const searchTiktok = async (keywords, limit = 10) => {
  let result = [];
  result = await searchTiktokTikwm(keywords, limit);
  result =
    result.length === 0 ? await searchTiktokGeneral(keywords, limit) : result;
  return result;
};
...
const getDataDownloadSubHatDe = async (url) => {
  try {
    const response = await retry(
      async () => {
        const res = await axios.get(`${linkTikwm}${encodeURIComponent(url)}`, {
          timeout: 10000,
        });
        if (res.data.code !== 0) {
          throw new Error(
            `API tikwm failed with code ${res.data.code}: ${res.data.msg}`,
          );
        }
        return res;
      },
      {
        ...
        maxTimeout: 2000,
        onRetry: (error) => {
          console.warn(`Retrying tikwm API due to error: ${error.message}`);
        },
      },
    );
    ...
    console.error("Error calling tikwm API:", error.message, {
      url,
  ...
  try {
    ...
    return data.map((item) => ({
      ...
      video: {
        ...
        duration: item.video.duration ? item.video.duration * 1000 : 1000,
        type: item.video.format || "mp4",
        quality: item.video.ratio || "540p",
      },
      author: item.author,
      music: {
    ...
    console.error("Error fetching related TikTok videos:", error.message);
    return [];
  }
};
...
export const getTiktokPreload = async () => {
  try {
    ...
    console.error("Error fetching preload TikTok videos:", error.message);
    return [];
  }
};
