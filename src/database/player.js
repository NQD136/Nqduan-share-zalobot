...
async function isUserBlocked(idUserZalo) {
  ...
  return (
    ...
    blockData.some((user) => user.idUserZalo === idUserZalo)
  );
}
...
export async function login(username, password, idUserZalo, senderName, api) {
  try {
    ...
    const [accountRows] = await connection.execute(
      `SELECT username FROM ${NAME_TABLE_ACCOUNT} WHERE username = ? AND password = ?`,
      [username, password],
    );
    ...
    if (accountRows.length === 0) {
      ...
      return {
        ...
        message: `Tên đăng nhập hoặc mật khẩu Không đúng. ${
          5 - loginAttempts[idUserZalo] === 0
            ? "Bạn đã bị khóa đăng nhập!"
            : `Bạn còn ${5 - loginAttempts[idUserZalo]} lần thử.`
        }`,
      };
    }
    ...
    const [playerRows] = await connection.execute(
      `SELECT idUserZalo, playerName FROM ${NAME_TABLE_PLAYERS} WHERE username = ?`,
      [accountUsername],
    );
    ...
    if (playerRows.length === 0) {
      ...
      return {
        success: true,
        message: `Đã đăng nhập lần đầu thành công. Bạn được tặng 10,000 VNĐ.`,
      };
    } else if (playerRows[0].idUserZalo === "-1") {
      await connection.execute(
        `UPDATE ${NAME_TABLE_PLAYERS} SET idUserZalo = ? WHERE username = ?`,
        [idUserZalo, accountUsername],
      );
      ...
      return {
        success: false,
        message: `Tài khoản đã đăng nhập bởi Zalo của '${zaloName}'.`,
      };
    }
  } catch (error) {
...
export async function logout(idUserZalo) {
  try {
    const [result] = await connection.execute(
      `UPDATE ${NAME_TABLE_PLAYERS} SET idUserZalo = '-1' WHERE idUserZalo = ?`,
      [idUserZalo],
    );
    ...
    if (result.affectedRows === 0) {
      return {
        success: false,
        message: `Bạn chưa đăng nhập bất kỳ tài khoản nào!`,
      };
    }
...
export async function isHaveLoginAccount(idUserZalo) {
  try {
    ...
    console.error(
      "Lỗi khi kiểm tra trạng thái đăng nhập của người chơi:",
      error,
    );
    throw error;
  }
...
export async function banPlayer(idUserZalo) {
  try {
    await connection.execute(
      `UPDATE ${NAME_TABLE_PLAYERS} SET isBanned = 1 WHERE idUserZalo = ?`,
      [idUserZalo],
    );
...
export async function unbanPlayer(idUserZalo) {
  try {
    await connection.execute(
      `UPDATE ${NAME_TABLE_PLAYERS} SET isBanned = 0 WHERE idUserZalo = ?`,
      [idUserZalo],
    );
...
export async function isPlayerBanned(idUserZalo) {
  try {
    const [rows] = await connection.execute(
      `SELECT isBanned FROM ${NAME_TABLE_PLAYERS} WHERE idUserZalo = ?`,
      [idUserZalo],
    );
...
export async function isPlayerActive(idUserZalo) {
  try {
    ...
    const [rows] = await connection.execute(
      `SELECT active FROM ${NAME_TABLE_ACCOUNT} WHERE username = ?`,
...
export async function claimDailyReward(idUser) {
  try {
    const [rows] = await connection.execute(
      `SELECT * FROM ${NAME_TABLE_PLAYERS} WHERE idUserZalo = ?`,
      [idUser],
    );
    ...
    const lastReward = player.lastDailyReward
      ? new Date(player.lastDailyReward)
      : null;
...
export async function getMyCard(api, idUser) {
  try {
    ...
    const winRate =
      player.totalGames > 0
        ? new Big(player.totalWinGames).div(player.totalGames).times(100)
        : new Big(0);
...
export async function setLoserGame(idUser, amount) {
  try {
    const [playerRows] = await connection.execute(
      `SELECT balance FROM ${NAME_TABLE_PLAYERS} WHERE idUserZalo = ?`,
      [idUser],
    );
    if (playerRows.length === 0) {
    ...
    return {
      success: false,
      message: `Đã xảy ra lỗi khi cập nhật lượt thua. ❌`,
    };
  }
}
...
export async function setLoserGameByUsername(username, amount) {
  try {
    const [playerRows] = await connection.execute(
      `SELECT balance FROM ${NAME_TABLE_PLAYERS} WHERE username = ?`,
      [username],
    );
    if (playerRows.length === 0) {
      return { success: false, message: `Không tìm thấy người chơi. ❌` };
    ...
    if (isWin !== null) {
      ...
      const positiveAmount =
        isSetWinPoint && numAmountWin
          ? numBalanceWin.toString()
          : bigNumAmount.gt(0)
            ? bigNumAmount.toString()
            : "0";
...
export async function getAccountVND(username) {
  try {
    const [rows] = await connection.execute(
      `SELECT vnd FROM ${NAME_TABLE_ACCOUNT} WHERE username = ?`,
      [username],
    );
...
export async function updateAccountVND(username, amount) {
  try {
    ...
    const [result] = await connection.execute(
      `UPDATE ${NAME_TABLE_ACCOUNT} SET vnd = ? WHERE username = ?`,
      [newBalance.toString(), username],
    );
...
export async function registerAccount(username, password) {
  try {
    const [existingUsers] = await connection.execute(
      `SELECT username FROM ${NAME_TABLE_ACCOUNT} WHERE username = ?`,
      [username],
    );
    ...
    await connection.execute(
      `INSERT INTO ${NAME_TABLE_ACCOUNT} (username, password) VALUES (?, ?)`,
      [username, password],
    );
...
export async function updatePlayerBalanceByUsername(
  username,
  amount,
  isWin = null,
  numAmountWin,
